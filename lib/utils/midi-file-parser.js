...
let MIDIFile = require('./midi-file-parser/midi-file');
let MIDIFileEvent = require('./midi-file-parser/midi-file/event');
...
function _parseChannelEvent(eventByte, reader) {
	...
	switch (eventByte & 0xF0) {
		...
		case MIDIFileEvent.MESSAGE_TYPE_NAMES_TO_NUMBERS.ProgramChange:
			eventData.programNumber = reader.nextUInt8();
			break;
...
function parse(buffer) {
	...
	if (reader.tell() < buffer.byteLength) {
		console.log('REMAINING BYTES: ', reader.restAll());
	}
